...
class Writer:
    ...
    def flush(self):
        if not self._chunk.empty():
            ...
            self._add_chunk_header_index(
                chunk_header_position, self._chunk._proto_chunk_header
            )
    ...
    def _need_split_chunk(self):
        return (
            self._chunk.size() >= self._header.chunk_raw_size
            or self._chunk.interval() >= self._header.chunk_interval
        )

    def _need_split_file(self):
        return (
            self._header.end_time - self._header.begin_time
            >= self._header.segment_interval
        ) or (self._header.size >= self._header.segment_raw_size)
