...
def cyber_record_info(record_file):
    ...
    print(
        "begin_time:  {}".format(datetime.fromtimestamp(record.get_start_time() / 1e9))
    )
    print("end_time:    {}".format(datetime.fromtimestamp(record.get_end_time() / 1e9)))
    print(
    ...
    for channel in record.get_channel_cache():
        print(
            "{:<38}, {:<38}, {}".format(
                channel.name, channel.message_type, channel.message_number
            )
        )
