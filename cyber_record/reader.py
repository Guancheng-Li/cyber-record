...
class Reader:
    ...
    def _fill_header(self, header):
        self.bag._version = "{}.{}".format(header.major_version, header.minor_version)
    ...
    def _get_chunk_body_indexs(self, start_time, end_time):
        for chunk_header_index, chunk_body_index in self.sorted_chunk_indexs:
            if (
                start_time
                and chunk_header_index.chunk_header_cache.end_time < start_time
            ):
                continue

            if end_time and chunk_header_index.chunk_header_cache.begin_time > end_time:
                continue
        ...
        for chunk_body_index in self._get_chunk_body_indexs(start_time, end_time):
            ...
            while not self.chunk.end():
                ...
                if self._is_valid_topic(
                    single_message.channel_name, topics
                ) and self._is_valid_time(single_message.time, start_time, end_time):
